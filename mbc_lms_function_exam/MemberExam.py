...
# c : 회원가입
# r : 회원리스트, 관리자인경우 회원암호 변경, 블랙리스트생성, 권한 부여
# r : 로그인 id와 pw를 활용하여 로그인 상탱 ㅠ지 session
# u : 회원정보 수정
# d : 회원탈퇴, 회원비활성화
...
아이디 : {new_id}  비빌먼호 : {new_pw}
""")
        save_select = input("저장하려면 y : ")
        if save_select == "y":
            print("저장을 시작합니다.")
            ids.append(new_id)  # .append()리스트 뒤에 추가
            pws.append(new_pw)
            names.append(new_name)
            roles.append(new_role)
            active.append(True) # 저장 완료
            print("저장 완료")
...
def member_login ():
    ...
    # is not None 싱글톤이라는 객체
    # if session != None -> 숫자 비교형 이퀄이냐는 값으로 사용
        # True
        print("이미 로그인 상태입니다.")
        ...
        if user_id in ids : # 키보드로 받은 id를 ids 리스트에 있는지??
            ...
            if not active [idx] : # 회원 활성화 상태인지?
                ...
                # True
                # 암호를 비교한다.
                if user_pw == pws[idx] : # 키보드로 넣은 암호와 주소 암호 일치
                    # id도 같고 활성화 상태이고 암호가 같다
                    session = idx # 로그인 상태로 완성
                    # 글로벌 영역에 세션값(로그인 사용자의 주소)이 있는 상태가 됨
...
def member_admin ():
    ...
    # 권한 부여 -> 사용자의 권한을 roles를 변경 (manage <-> user)
    print("member_admin 함수를 종료합니다.")
 # 관리자가 사용자 변경사항 함수 종료

def member_logout ():
    # 회원 로그아웃으로 상태 변경 -> session 값은 None 으로 변경
